...
* **Phương pháp**: Dữ liệu khảo sát từ $N = 126$ sinh viên CNTT tại các trường đại học lớn được làm sạch, kiểm định và phân tích thông qua quy trình thống kê chuẩn mực (cleaned, validated, and analyzed using standard statistical procedures) dựa trên mô hình chấp nhận công nghệ (UTAUT) và khung đo lường DORA.
* **Kết quả**: Nghiên cứu xác nhận thực trạng nhận thức và thực hành CI/CD của sinh viên ở mức thấp: 82.5% sinh viên có nhận thức mơ hồ (38.9% chưa từng nghe, 43.7% chỉ nghe tên) và 60.3% chưa từng sử dụng công cụ. Đối với nhóm đã sử dụng ($N = 50$), ngữ cảnh áp dụng chủ yếu là đồ án môn học/dự án cá nhân (32.0%), nhưng có tới 68.0% nhóm này thực tế vẫn chưa hiểu rõ bản chất (tiếp cận thụ động qua cấu hình có sẵn). Đối với nhóm chưa dùng ($N = 76$), rào cản cốt lõi là thiếu kiến thức/kỹ năng (43.8%) và thiếu cơ hội thực hành trong môn học (32.9%). Kiểm định tương quan Pearson chỉ ra Điều kiện hỗ trợ (FC) ảnh hưởng trực tiếp mạnh mẽ đến hành vi sử dụng ($r = 0.610, p < 0.001$).
...
* **Method**: Survey data from $N = 126$ IT students were cleaned, validated, and analyzed using standard statistical procedures, integrating the Unified Theory of Acceptance and Use of Technology (UTAUT) and DORA metrics.
* **Results**: The results reveal extremely low awareness and practice: 82.5% of students report low awareness (38.9% never heard of CI/CD, 43.7% only heard the name), and 60.3% have never used these tools. Among active users ($N = 50$), the primary context is course/personal projects (32.0%), yet 68.0% of this active group still do not understand it clearly (indicating passive usage via pre-configured setups). For non-users ($N = 76$), the core barriers are lack of knowledge/skills (43.8%) and lack of opportunities in university courses (32.9%). Pearson correlation confirms that Facilitating Conditions (FC) strongly predict actual use behavior ($r = 0.610, p < 0.001$).
...
1.  Cung cấp bằng chứng thực nghiệm định lượng chính xác về thực trạng nhận thức thấp (82.5%) và tỷ lệ không áp dụng (60.3%) trong mẫu khảo sát $N = 126$ sinh viên CNTT Việt Nam.
2.  Phát hiện và lý giải "nghịch lý thụ động" ở nhóm đã sử dụng ($N=50$): phần lớn (68.0%) chỉ sử dụng lại cấu hình có sẵn hoặc do bạn trong nhóm làm hộ mà chưa thực sự hiểu bản chất.
...
| **Ref03** (ACM, 2020) | Thách thức của SV khi học DevOps | Định tính, khảo sát | Cỡ mẫu nhỏ, chưa kiểm định tương quan thống kê | Sử dụng kiểm định độc lập T-test và Pearson đối với $N=126$ sinh viên |
...
| **RQ2**: Đối với nhóm đã sử dụng ($N=50$), họ học từ đâu, áp dụng trong ngữ cảnh và bằng công cụ nào? | Tìm ra nguyên nhân và động lực giúp nhóm sinh viên này tiếp cận công nghệ. | Q7 (Công cụ), Q8 (Nguồn học), Q15-17 (Likert động lực) |
...
Quy mô mẫu cuối cùng đạt được là **$N = 126$ sinh viên**. Cỡ mẫu này hoàn toàn đảm bảo lực lượng thống kê cho mô hình hồi quy đa biến và kiểm định chéo dựa trên:
1.  **Quy tắc Green (1991)**: Yêu cầu tối thiểu là $50 + 8 \times 4 = 82$ cho mô hình có 4 biến độc lập. Quy mô $N=126$ vượt xa mức tối thiểu này.
...
Mẫu khảo sát gồm $N=126$ sinh viên, phân bố chủ yếu ở khối Năm 3 và Năm 4 ($73.1\%$) - bối cảnh sinh viên bắt đầu làm nhiều đồ án chuyên ngành phức tạp và đi thực tập doanh nghiệp.
...
*   **Mức độ nhận thức (Q5)**: **82.5%** sinh viên tự đánh giá ở mức thấp hoặc chưa biết gì. Cụ thể: 38.9% chưa từng nghe nói đến CI/CD; 43.7% có nghe tên qua nhưng không hiểu rõ bản chất. Chỉ có 16.7% biết khái niệm cơ bản và vỏn vẹn 0.8% hiểu rõ để tự cấu hình.
*   **Thực hành áp dụng**: **60.3%** (76 sinh viên) chưa từng chạm tay vào công cụ CI/CD, chỉ có **39.7%** (50 sinh viên) đã từng áp dụng trong đồ án của mình.
...
#### C. Kết quả RQ2: Ngữ cảnh, Nguồn học và Động lực của nhóm đã sử dụng ($N = 50$)
Đối với nhóm sinh viên đã từng áp dụng công cụ CI/CD:
1.  **Ngữ cảnh & Nguồn tiếp cận (Q8)**:
    - 32.0% (16/50 sinh viên) biết đến qua **Dự án cá nhân / Đồ án môn học**.
    - 28.0% (14/50 sinh viên) tự tìm hiểu trên Internet.
    - Chỉ có 20.0% (10/50 sinh viên) học từ **Môn học chính thức tại trường**.
    - Thực tập tại doanh nghiệp chỉ đóng góp 14.0% (7/50 sinh viên).
...
1.  **Nhóm chưa từng sử dụng công cụ ($N = 76$)**:
    - Rào cản lớn nhất là **Thiếu kiến thức hoặc kỹ năng về CI/CD/DevOps** chiếm **43.8%** (39/76 sinh viên).
    - **Ít cơ hội thực hành trong môn học tại trường** chiếm **28.1%** (25/76 sinh viên).
    - **Dự án học tập còn đơn giản nên chưa thấy cần thiết** chiếm **18.0%** (16/76 sinh viên).
2.  **Nhóm đã biết lý thuyết nhưng không dùng ($N = 6$)**:
    - **Thiếu kiến thức/kỹ năng chuyên sâu**: **62.5%** (5/6 sinh viên).
    - **Thiết lập và cấu hình công cụ quá phức tạp**: **50.0%** (4/6 sinh viên).
    - **Thiếu môi trường thực hành (server, cloud, Docker)**: **37.5%** (3/6 sinh viên).
3.  **Phân tích tương quan UTAUT (FC $\rightarrow$ Use)**:
    Kiểm định tương quan Pearson chỉ ra mối quan hệ đồng biến cực mạnh giữa **Điều kiện hỗ trợ (FC)** và **Hành vi sử dụng thực tế (Use)** ($r = 0.610, p < 0.001$). Tương tự, **Tự hiệu quả (Self-efficacy)** tương quan mạnh mẽ với hành vi ($r = 0.611, p < 0.001$). Kết quả toán học này chứng minh: sự e ngại cấu hình và thiếu hụt hạ tầng máy chủ miễn phí/bài Lab hướng dẫn trực quan là nguyên nhân trực tiếp khiến sinh viên không thể đưa ý định học tập thành hành động thực tế.
...
#### Findings
Kết quả phân tích định lượng xác nhận thực trạng nhận thức CI/CD của sinh viên Việt Nam còn rất thấp (82.5% mơ hồ) và tồn tại khoảng cách Knowing-Doing lớn. Rào cản không nằm ở động lực cá nhân của sinh viên, mà nằm ở **Điều kiện hỗ trợ (FC)** của nhà trường (chỉ 20.0% học qua môn học chính thức). Sự phức tạp khi cấu hình công cụ kết hợp với sự thiếu hụt máy chủ/Docker thực hành đẩy sinh viên vào trạng thái "sử dụng thụ động" (68.0% dùng nhưng không hiểu bản chất).
...
2.  **Đặc thù cỡ mẫu**: Nghiên cứu khảo sát trên $N = 126$ sinh viên tại Việt Nam. Do đó, kết quả này có thể không đại diện và khái quát hóa hoàn toàn cho cộng đồng kỹ sư DevOps chuyên nghiệp tại các doanh nghiệp (The findings may not generalize to professional DevOps practitioners).
...
1.  **Kết luận 1**: Tỷ lệ sinh viên có nhận thức thấp về CI/CD lên tới 82.5%, và 60.3% chưa bao giờ thực hành công cụ.
2.  **Kết luận 2**: Phần lớn nhóm sinh viên đã dùng CI/CD (68.0%) chỉ tiếp cận ở mức độ thụ động thông qua các đồ án nhóm mà chưa làm chủ kỹ năng tự cấu hình.
